...
\textbf{Solution:} Here we see all positive observations and with an integral equal to 1, which tells us we are working with a valid PDF. 
	<<>>=
x <- seq(-5,5,0.001)	
...
\textbf{Solution:} Below it is clear that as the sample size increases, the histograms of the observations get closer to the true density function at the specified parameters. Eventually with a very large sample and very small bins for the histogram, they would look essentially the same. We see the same trend regardless of which parameter set is used. 
<<>>= 
library(patchwork)
