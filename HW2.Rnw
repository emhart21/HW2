...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  \textbf{Solution:}
  The Bernoulli distribution is a special case of the binomial distribution, in which each observation can take one of two outcomes.  We define these as a ``success," where the observation of interest occurs, or a ``failure," where it does not.  Thus, the support for this distribution (or ``the set of all possible values of our random variable") is $\chi = \{x \in \{0,1\}\}$ (Chapter 6 Notes, 163).  We use p to denote the probability that a ``success" occurs and $q=1-p$ to denote the probability that a ``failure" occurs. Thus, the probability mass function for the Bernoulli distribution is given by $f_X(x|p)=p^x(1-p)^{1-x}$, where x is either 0 or 1 (Sinharay, 132).  We can write this to be robust to other inputs by including an indicator function, I, such that the PMF is given by $f_X(x|p)=p^x(1-p)^{1-x}I(x\in\{0,1\})$ (Cipolli, 173). The cumulative distribution function of the Bernoulli distribution is given by:
$$ F_X(x|p) = 
\begin{cases} 
      0 & x < 0 \\
      1-p & 0\leq x <1 \\
...
# g1<-ggplot(NULL, aes(x=x.10.50$rbern.10..0.5.)) +       #which data to plot
#   geom_histogram(data=x.10.50, breaks=seq(-0.5,1.5,1),                #Set up bins to use
#                  fill = "lightgrey", color="black") +   #color the histogram
#   geom_linerange(data = ggdat, aes(ymax = f), ymin = 0) +
#   xlab("X") +                                           #x axis label
#   ylab("Frequency")+                                    #y axis label
#   ggtitle("Random Bernoulli Distribution with p=.5") +  #add title to plot
#   theme_bw()+                                           #removes grey background 
#   geom_hline(yintercept=0) +                            #adds a line for the x-axis
#   xlim(-1,2)                                            # show the full density plot

g1<-ggplot(data=x.10.50) +       #which data to plot
  geom_histogram(aes(x=rbern.10..0.5.), breaks=seq(-0.5,1.5,1),                #Set up bins to use
                 fill = "lightgrey", color="black") +   #color the histogram)
  #geom_linerange(aes(x=c(0,1), ymax=c(5,5), ymin=0))
  xlab("X") +                                           #x axis label
  ylab("Frequency")+                                    #y axis label
  ggtitle("Random Bernoulli Distribution with p=.5") +  #add title to plot
  theme_bw()+                                           #removes grey background 
...
g2<-ggplot(data=x.25.50, aes(x=rbern.25..0.5.)) +       #which data to plot
  geom_histogram(breaks=seq(-0.5,1.5,1),                #Set up bins to use
                 fill = "lightgrey", color="black") +   #color the histogram)  
  xlab("X") +                                           #x axis label
  ylab("Frequency")+                                    #y axis label
  ggtitle("Random Bernoulli Distribution with p=.5") +  #add title to plot
...
g3<-ggplot(data=x.100.50, aes(x=rbern.100..0.5.)) +       #which data to plot
  geom_histogram(breaks=seq(-0.5,1.5,1),                #Set up bins to use
                 fill = "lightgrey", color="black") +   #color the histogram)  
  xlab("X") +                                           #x axis label
  ylab("Frequency")+                                    #y axis label
...
g4<-ggplot(data=x.1000.50, aes(x=rbern.1000..0.5.)) +       #which data to plot
  geom_histogram(breaks=seq(-0.5,1.5,1),                #Set up bins to use
                 fill = "lightgrey", color="black") +   #color the histogram)  
  xlab("X") +                                           #x axis label
  ylab("Frequency")+                                    #y axis label
...
g5<-ggplot(data=x.10.75, aes(x=rbern.10..0.75.)) +       #which data to plot
  geom_histogram(breaks=seq(-0.5,1.5,1),                #Set up bins to use
                 fill = "lightgrey", color="black") +   #color the histogram)  
  xlab("X") +                                           #x axis label
  ylab("Frequency")+                                    #y axis label
  ggtitle("Random Bernoulli Distribution with p=.75") +  #add title to plot
  theme_bw()+                                           #removes grey background 
  geom_hline(yintercept=0) +                            #adds a line for the x-axis
  xlim(-1,2)                                            # show the full density plot

g6<-ggplot(data=x.25.75, aes(x=rbern.25..0.75.)) +       #which data to plot
  geom_histogram(breaks=seq(-0.5,1.5,1),                #Set up bins to use
                 fill = "lightgrey", color="black") +   #color the histogram)  
  xlab("X") +                                           #x axis label
  ylab("Frequency")+                                    #y axis label
...
g7<-ggplot(data=x.100.75, aes(x=rbern.100..0.75.)) +       #which data to plot
  geom_histogram(breaks=seq(-0.5,1.5,1),                #Set up bins to use
                 fill = "lightgrey", color="black") +   #color the histogram)  
  xlab("X") +                                           #x axis label
  ylab("Frequency")+                                    #y axis label
...
g8<-ggplot(data=x.1000.75, aes(x=rbern.1000..0.75.)) +       #which data to plot
  geom_histogram(breaks=seq(-0.5,1.5,1),                #Set up bins to use
                 fill = "lightgrey", color="black") +   #color the histogram)  
  xlab("X") +                                           #x axis label
  ylab("Frequency")+                                    #y axis label
...
@
  The greater the size of our random sample, the closer, generally, the observations we make are to the number we would predict from the 
	\end{enumerate}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%  Question 4 
...
bern.summ(0.75)
@
In the case of the Bernoulli distribution, the mean of the PMF is the parameter p.
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  %%%%%%%%%  Part (b)
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  \item Generate a random sample of size $n=10, 25, 100$, and $1000$ for your 
